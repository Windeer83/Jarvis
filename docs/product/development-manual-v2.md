...
> 状态：已确认需求基线并完成 #31 真机技术门禁，2026-09-01。V2-A 在 Mate 70 Pro+ / HarmonyOS 4.3 上采用已实测的“尽力型覆盖阻断”，不声称普通应用拥有系统级暂停其他应用的能力；真实通知入口、省电模式和 8 小时稳定性由用户明确免测并作为已接受风险保留。本文是第二版实施权威；`supervision-spec.md`、`companion-spec.md` 与 `growth-and-knowledge-spec.md` 继续保存第一版完整基线，冲突时以本文明确写出的 V2 变更为准。
...
[HarmonyOS 4.3 手机监督端可行性研究](../research/huawei-mate70pro-plus-harmonyos43-supervision-feasibility.md)确认：普通非 root、非 Device Owner、非企业 MDM 应用没有官方系统级暂停任意其他包的权限。#31 真机 spike 已验证 `UsageStatsManager` 前台识别、`TYPE_APPLICATION_OVERLAY` 全屏覆盖、可见前台服务、本地 alarm、离线策略缓存和 `BOOT_COMPLETED` 恢复的最小组合；产品只能称为“尽力型应用阻断”。无障碍对照在相同矩阵中出现漏报并会被华为停用，正式骨架不得依赖。
...
- `UsageStatsManager` 在 100 次受控切换中识别目标应用无漏报；
- 普通全屏 overlay 在桌面、真实深链和华为最近任务入口完成阻断；
- 五分钟临时开放仅作用于当前应用，到期恢复阻断；
- 断网、无 Windows、息屏和从最近任务清除后继续执行；
- 真机重启后保留原策略和结束时间，通过 `BOOT_COMPLETED` 重建 exact alarm 与前台服务，恢复后四个目标应用均被阻断。

重启命令到恢复广播之间实测存在约 109 秒空窗，其中包含关机、开机、首次解锁和系统广播延迟。正式产品在手机重新报告健康前必须显示降级，不能声称该空窗内已经阻断。真实通知入口、华为省电模式和 8 小时稳定性为用户明确接受的免测风险，不得在文档或 UI 中写成已通过。权限撤销/系统终止的降级检测，以及同局域网配对、策略同步、离线事件补传和幂等去重，仍由正式实现及其验收 Issue 完成。
...
- 四个目标应用分别从桌面、真实深链和最近任务启动至少 25 次，每次都在用户完成有效交互前出现阻断层；真实通知入口按用户决定不作为当前门禁。
- Windows 断网或关机后继续执行已确认策略；原定结束后 10 秒内解除，不等待电脑消息。
- 重启后保留原策略 ID 和结束时间，自动重建定时与前台服务；恢复健康前电脑端必须显示监督降级。
- 华为省电模式和 8 小时稳定性按用户决定不作为当前门禁；这两项保持“未测且接受风险”，不能改写为通过。
